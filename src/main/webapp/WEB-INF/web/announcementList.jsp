                        ...
                        <div id="tg-content" class="tg-content">
                            ...
                            <div class="tg-newsandarticle tg-list">
                                <table id="fileListView" class="table table-hover">
                                    ...
                                    <c:forEach items="${pagedAnnouncement}" var="announcement">
                                        <tr>
                                            <td class="col-xs-2" style="border-right: 0;border-left: 0;border-top: 0"><c:out value="${announcement.date}"></c:out></td>
                                            <td class="col-xs-2" style="border-right: 0;border-left: 0;border-top: 0"><c:out value="${announcement.title}"></c:out></td>
                                            <td class="col-xs-1" style="border-right: 0;border-left: 0;border-top: 0"><button class="label label-info"><a href="/tanklab/detailAnnouncement?id=${announcement.id}">More Info</a></button></td>
                                        </tr>
                                    </c:forEach>
                                    </tbody>
                                </table>
                                <nav class="tg-pagination">
                                    <ul>
                                        <%--page不等于1显示首页--%>
                                        <c:if test="${page != 1}">
                                            <li class="tg-prevpage"><a href="/tanklab/announcementList?page=1">首页</a></li>
                                        </c:if>
                                        <%--前一页不为空显示上一页--%>
                                        <c:if test="${prePageIndex != null}">
                                            <li class="tg-prevpage"><a href="/tanklab/announcementList?page=${prePageIndex}">上页</a></li>
                                        </c:if>
                                        <%--遍历列表--%>
                                        <c:forEach var="index" begin="1" end="${maxPage}" step="1">
                                            <c:choose>
                                                ...
                                                <c:when test="${page == index}">
                                                    <li class="tg-active"><a href="/tanklab/announcementList?page=${index}">${index}</a></li>
                                                </c:when>
                                                <%--否则正常输出--%>
                                                <c:otherwise>
                                                    <li><a href="/tanklab/announcementList?page=${index}">${index}</a></li>
                                                </c:otherwise>
                                            </c:choose>
                                        </c:forEach>
                                        <%--前一页不为空显示上一页--%>
                                        <c:if test="${nextPageIndex != null}">
                                            <li class="tg-nextpage"><a href="/tanklab/announcementList?page=${nextPageIndex}">下页</a></li>
                                        </c:if>
                                        <%--page不等于maxPage显示尾页--%>
                                        <c:if test="${page != maxPage}">
                                            <li class="tg-nextpage"><a href="/tanklab/announcementList?page=${maxPage}">尾页</a></li>
                                        </c:if>
                                    </ul>
                                </nav>
                            </div>
