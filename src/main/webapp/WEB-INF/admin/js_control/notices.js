$(function(){
	...
	var refreshAllList = function() {
		$.ajax({
			url:'/tanklab/api/v1/notices?page=1',
			method:'GET',
			success:function(restMessage) {
				if(restMessage.code == 200) {
					//清空tbody里的所有行
                    $('#notices_form_body').html('');
					for(var i = 0; i < restMessage.data.length; i++) {
						var noticesObj = restMessage.data[i];
						//append遍历添加每一行
						$('#notices_form_body').append("<tr>\n" +
                            "                  <td class=\"col-xs-1\" id=\"index_" + (i+1) + "\">" + (i+1) + "</td>\n" +
                            "                  <td class=\"col-xs-1\" id=\"notices_id_" + noticesObj.id + "\">" + noticesObj.id + "</td>\n" +
                            "                  <td class=\"col-xs-2\" id=\"notices_date_" + noticesObj.id +"\">" + timeStamp2String(noticesObj.date) + "</td>\n" +
        ...
        $('button[btnType="delButton"]').each(function() {
            ...
            $(this).on('click', {id:param_del_id}, function(event) {
                ...
                if(confirm_message == true) {
                    $.ajax({
                        url: "/tanklab/api/v1/notices?id=" + event.data.id,//请求异步的API路径加上变量
                        type: "DELETE",//请求类型，删除的话是DELETE
                        success: function(data) {
                            //根据返回的JSON中的code字段是否为200判断是否删除成功，并给用户提示
                            if(data.code == 200) {
                                $("#del_notices_" + event.data.id).parent().parent().hide(1500);
                                alert('删除成功');
                            }else {
                                alert('操作失败，请稍后再试');
                            }
    ...
    var bindingEveryUpdateButton = function() {
        $('button[btnType="chgButton"]').each(function() {
            ...
            $(this).on('click', {
                id:$("#notices_id_" + chg_id).text(),
                title:$("#notices_title_" + chg_id).text(),
                content:$("#notices_content_" + chg_id).html(),//由于首页使用substring()，因此为了读到全文，在该表格的一个属性中封装字段，使其能读出，修改全文
                date:$("#notices_date_" + chg_id).text()
            }, function(event) {
                var chg_data = event.data;//获取传参数的对象
                $('#change_notices_modal').modal();//调用弹出模态框
    ...
    $('#change_submit').on('click', function() {
        ...
        $('#change_notices_form').ajaxSubmit({
            success: function(restMessage) {
                if(restMessage.code == 200) {
                    alert('修改成功')
                    $('#change_notices_modal').modal('hide');
                    refreshAllList();
                } else {
                    alert('修改失败，请稍后再试');
                }
